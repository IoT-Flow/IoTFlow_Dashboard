...
const CustomChart = ({
  ...
  const fetchChartData = async () => {
    if (
      !chartConfig.devices ||
      !chartConfig.devices.length ||
      !chartConfig.measurements ||
      !chartConfig.measurements.length
    ) {
    ...
    try {
      ...
      for (const deviceId of chartConfig.devices) {
        try {
          ...
          console.debug(
            `[CustomChart] Received ${telemetryRecords.length} records for device ${deviceId}`
          );
          ...
          chartConfig.measurements.forEach(measurement => {
            ...
            if (measurementData.length > 0) {
              ...
              console.debug(
                `[CustomChart] Added ${measurementData.length} points for ${measurement} from device ${deviceId}`
              );
            }
          });
        } catch (deviceError) {
      ...
      if (allTelemetryData.length === 0) {
        const deviceNames = chartConfig.devices
          .map(deviceId => {
        ...
        setError(
          `No data found for devices [${deviceNames}] and measurements [${chartConfig.measurements.join(', ')}] in the selected time range (${chartConfig.timeRange}). Try selecting a longer time range or check if the devices have recent data.`
        );
      }
    } catch (err) {
          ...
          [
            ...
            },
            {
              ...
              color: echarts.color.modifyAlpha(
                chartConfig.customColors?.[idx] || colorPalette[idx % colorPalette.length],
                0.7
              ),
            },
          ]
  ...
  const generateRadarChartOption = (telemetryArrays, isDarkMode) => {
    ...
    const measurements = chartConfig.measurements || [
      'Value 1',
      'Value 2',
      'Value 3',
      'Value 4',
      ...
      return (
        ...
        >
          ...
          <Alert severity="info" sx={{ maxWidth: 300 }}>
            This represents a preview of advanced IoT dashboard widgets. Interactive features and
            real-time controls coming soon.
          </Alert>
      ...
      <Card sx={{ height: isFullscreen ? '100vh' : 400 }}>
        <CardContent sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
          ...
          <Box display="flex" flexWrap="wrap" gap={0.5} mb={2}>
            <Chip label={chartConfig.type} size="small" variant="outlined" color="primary" />
            <Chip label={chartConfig.timeRange} size="small" variant="outlined" />
            {chartConfig.devices?.length > 0 && (
              <Chip
  ...
  return (
    ...
    >
      <CardContent
        sx={{ flexGrow: 1, pb: 1, display: 'flex', flexDirection: 'column', width: '100%' }}
      >
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <Box>
            <Typography variant="h6" component="h3" sx={{ fontWeight: 600 }}>
